...
更新中控二进制并重启后，在网页选一个已完成的录像片段，依次拖到后半段和前半段，确认画面及播放时间随之变化。网络面板应出现 `/api/recordings/play` 的 HLS 列表，以及 `/api/recordings/segment` 的短分片请求。支持 MSE 的浏览器优先使用内置 HLS.js；不支持 MSE 时回退到浏览器原生 HLS，无需访问外部 CDN。Actions 用 FFmpeg 生成实际录像，经过 MediaMTX 1.20.0 和 Google Chrome 验证桌面、窄屏播放、跨片段连续播放、前后拖动、重试和快速切换。
